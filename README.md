...
- **Module B (Extractor)**: 物理特徴量（MFCC, Spectral, Theoretical, Chroma）の抽出・標準化
- **Module C (Label Manager & API)**: WebベースのラベリングUI（複数人の `subjectId` に対応）およびExpress APIサーバー。さらに独立したダウンローダー専用UI(`public/downloader.html`)も含む
...
- **Module C (node/server.js, node/public/)**
  ...
  - **API Server (`server.js`)**: イベントループをブロックしないよう、完全に `fs/promises` による非同期I/Oを採用しています。また、`ALLOWED_ORIGINS`（デフォルト: `http://localhost:5173`）を利用したCORS制御を備えています。エンドポイントへのリクエストには `subjectId` を含めることで、複数ユーザーのラベルデータを個別に管理します。
...
- **Module D (python/train.py, python/predict.py)**
  ...
  - **Trainer**: MLPRegressor等を用い、`dataset.json` (X) と `labelset.json` (Y) からモデルを学習します。データサイズに応じたSolverの自動選択（`lbfgs` / `adam`）や、Lossの改善停滞（< 1e-5が10回連続）を検知する独自の自動収束判定（Early Stopping）が実装されています。学習済みモデルは `model.pkl` に、学習履歴は `train_meta.json` に保存されます。Epoch毎のLossや収束状態をJSON形式で標準出力し、NodeサーバーのSSEストリーミングに渡します。学習途中での安全な中断（SIGTERMによる途中保存）や、既存モデルからの学習再開（`resume`, `warm_start=True`）にも対応しています。
...
- **Tools (Downloader, VAD, Processor)**
  ...
  - **`main.py`**: YouTube等のURLから音声をダウンロードし、Librosa等を用いて楽曲構造（イントロ、Aメロ、サビなど）をAgglomerative Clusteringやチャプター情報から自動解析し、分割します。また、クロマ特徴テンプレートマッチングにより楽曲全体の調とコード進行を推定し、`dataset.json` の `global_key`, `global_chords` や `segments.json` にメタデータとして保存します。
...
- **主観ラベル Y (`data/labelset.json`)**:
  ...
  - Module C のUIで生成されファイルに保存されます。Trainer等では特定ユーザーのラベル（`subjectId`）として複数人のラベリングデータを区別して保存できます。UIコンポーネント（checkboxes等）の値は `0.0` または `1.0` のフラットな浮動小数点配列としてエンコードされます。
- **学習済みモデル・状態**:
