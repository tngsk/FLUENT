...
- **主観ラベル Y (`data/labelset.json`)**:
  ...
  - Module C のUIで生成されファイルに保存されます。分析しやすいよう、ラベル配列とアンケート回答をセットで保存します。Trainer等では特定ユーザーのラベルとして利用されます。UIコンポーネント（checkboxes等）の値は `0.0` または `1.0` のフラットな浮動小数点配列としてエンコードされます。
- **学習済みモデル・状態**:
