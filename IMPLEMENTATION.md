...
学習は UI 経由ではなく CLI または API を使用して実行します。UI の `Model Training` セクションは廃止されました。

- CLI 実行例（ローカル端末）:

```bash
uv run python python/train.py --alpha 0.01
```
...
`pending` のセグメントが存在する場合、ユーザーに再学習を推奨するメッセージ（例: "N pending — retrain recommended"）が表示されます。学習は CLI/API 経由で実行してください。
...
{ "cols": 8, "data": { "example-001": [...] } }
```

`config.json` でラベルの次元構成を定義する。現在の構成（cols=8）：

| フィールド       | type         | 次元数                |
| ---------------- | ------------ | --------------------- |
| color            | color-picker | 3（RGB）              |
| liking           | slider       | 1（好きな度合い）     |
| brightness       | slider       | 1（曲の明るさ）       |
| arousal          | slider       | 1（覚醒度）           |
| imagery_clarity  | slider       | 1（イメージの鮮明さ） |
| color_confidence | slider       | 1（色選択の確信度）   |
